...
- **Eligible walls:** only walls on the **building envelope** (outermost horizontal/vertical axis over all walls with span ≥ 12 px, ignoring point segments). This includes interior-tagged perimeter walls (Phase 3 recall) and **excludes** interior walls mis-tagged `is_exterior` that sit away from the envelope edge. Envelope/interior scans require opening corroboration (`strict_open`). When the envelope cannot be computed (incomplete wall graph), falls back to `is_exterior` only.
- **Over-split control (crop_mode):** higher jamb peak threshold (`JAMB_INK_PEAK_FRAC_CROP`), minimum split segment width (`SPLIT_MIN_SEGMENT_FT_CROP`), touch-merge of narrow adjacent fragments (`_merge_over_split_fragments`), and rejection of horizontal spans wider than `HORIZ_MAX_FT_CROP` (long-wall false positives).
...
- **Crop-mode over-split controls (low px/ft plans):** higher jamb peak threshold, discard split fragments below 3.25 ft, touch-merge narrow touching fragments, reject horizontal detections wider than 8.75 ft, **mullion-gap merge** for vertical curtain-wall openings split by a structural jamb, and **precision filters** for repeating wall-fill hatch (CMU/storefront hash) plus weak east-facade sills when `px_per_ft` is below ~22. Short interior coaxial stubs (< 5 ft, `is_exterior=false`) on the envelope axis are not scanned.
